# -*- coding: utf-8 -*-
"""
Created on Mon Feb 14 11:59:01 2022
...
output_cell_cycle_length_file_name='Cell_cycle_length_matrix_ctr' #input cell cycle length file name
output_cell_division_timing_file_name='Cell_division_timing_matrix_ctr' #input cell division timing file name
output_Cell_proliferation_number_file_name='Cell_proliferation_number_ctr' #input cell proliferation file name
...
for each_emb in raw_data_list:
    ...
    emb_output_ = emb_output_[['time']]
        
    emb_cell_list = list(emb_output_.index.values)
    ...
    emb_output_cycle = emb_output_.max(level='cell_name') - emb_output_.min(level='cell_name') + 1 
    
    emb_output_cycle = emb_output_cycle.loc[middle_cell_cell_list]
        
    cell_number = pd.DataFrame(emb_output_['time'].value_counts())
    
    cell_number.rename(columns={'time':'cell_number'}, inplace = True)
    
    cell_numbers = list(cell_number.cell_number)
    
    for cell in cell_numbers:
    
        if cell == 5:
        
            FistTimePoint = min(cell_number[cell_number.cell_number==5].index)
            break
        else:
        
            FistTimePoint = min(cell_number[cell_number.cell_number==6].index)
    
    emb_output_divition = emb_output_.max(level='cell_name') - FistTimePoint + 1
    
    emb_output_divition = emb_output_divition.loc[middle_cell_cell_list]
        
    cell_proliferation = cell_number[cell_number.cell_number>=5]
    
    cell_proliferation = cell_proliferation.sort_values(by='cell_number', ascending=True)
               
    cell_proliferation.index = np.arange(len(cell_proliferation))   #出现5或6个细胞时为第零个时间点 
    
    if type(output_cellcycle) == list:
        output_cellcycle = emb_output_cycle
    else:
        output_cellcycle = pd.concat([output_cellcycle, emb_output_cycle], axis=1)
      
    if type(output_celldivition) == list:
        output_celldivition = emb_output_divition
    else:
        output_celldivition = pd.concat([output_celldivition, emb_output_divition], axis=1)
    
    if type(output_cellproliferation) == list:
        output_cellproliferation = cell_proliferation
    else:
        output_cellproliferation = pd.concat([output_cellproliferation, cell_proliferation], axis=1)
    
    a+=1
    print(a)
                              
output_cellcycle.columns = embryo_name
output_celldivition.columns = embryo_name
output_cellproliferation.columns = embryo_name
...
output_cellcycle=output_cellcycle.loc[Cell_list_353]*1.25
output_celldivition=output_celldivition.loc[Cell_list_357]*1.25

output_cellcycle.to_csv(os.path.join(Output_Folder,output_cell_cycle_length_file_name+'.txt'), sep='\t')
output_celldivition.to_csv(os.path.join(Output_Folder,output_cell_division_timing_file_name+'.txt'), sep='\t')
output_cellproliferation.to_csv(os.path.join(Output_Folder,output_Cell_proliferation_number_file_name+'.txt'), sep='\t')
