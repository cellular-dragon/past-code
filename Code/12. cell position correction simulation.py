...
def stage_cell_list(children_cell):#根据细胞找到其所在发育阶段的全部细胞
    cell=children_cell
    if cell in cell_list_AB16:
        stage_cell_list=list(cell_list['AB16'].dropna())    
    elif cell in cell_list_AB32:
        stage_cell_list=list(cell_list['AB32'].dropna())  
    elif cell in cell_list_AB64:
        stage_cell_list=list(cell_list['AB64'].dropna())  
    elif cell in cell_list_AB128:
        stage_cell_list=list(cell_list['AB128'].dropna())   
    elif cell in cell_list_AB256:
        stage_cell_list=list(cell_list['AB256'].dropna())          
...
##############################################################################
#1.input the follow files from  "9. relative cell position phenotype analysis"
...
##2.input the RNAi_embryo_length file from “3. extract relative cell position distance matrix”
RNAi_embryo_length = pd.read_csv(r'C:\Users\dell\Desktop\output_folder\RNAi_embryo_length.txt', index_col=0, sep='\t')

###3.input the path of distance matrix of control from “3. extract relative cell position distance matrix”
...
Embryo_cell_dist=[]
Embryo_cell_std=[]
Embryo_dist_sum = np.zeros(shape=(len(cell_list),len(cell_list)))#更改细胞列表
Total_matrix=[]
n=0
...
for i in range(len(position_info.index)):
    ...
    new_XYZ=[99.0 + 0.6*I,99.0 + 0.6*I,99.0 + 0.6*I]#复数        
