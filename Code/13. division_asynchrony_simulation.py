# -*- coding: utf-8 -*-
"""
Created on Tue Feb 22 21:58:18 2022
...
##############################################################################
###############################################################################################################################
#input the cell cycle length and division time data of control and RNAi from "1. extract cell cycle length and division timing"
Cellcycle_CTR = pd.read_csv(r'C:\Users\dell\Desktop\output_folder\Cell_cycle_length_matrix_ctr.txt', index_col=0, sep='\t')
Celldivition_CTR= pd.read_csv(r'C:\Users\dell\Desktop\output_folder\Cell_division_timing_matrix_ctr.txt', index_col=0, sep='\t')
Cellcycle_RNAi_matrix = pd.read_csv(r'C:\Users\dell\Desktop\output_folder\Cell_cycle_length_matrix_RNAi.txt', index_col=0, sep='\t')
Celldivition_RNAi_matrix= pd.read_csv(r'C:\Users\dell\Desktop\output_folder\Cell_division_timing_matrix_RNAi.txt', index_col=0, sep='\t')


##input the files from "10. embryo proliferation slope analysis"
RNAi_slope = pd.read_csv(r'C:\Users\dell\Desktop\output_folder/RNAi_slope.txt', index_col=0, sep='\t')

###input the follow files from '6. extract division timing matrix and division asynchrony phenotype analysis'
division_asynchrony_phenotype=pd.read_csv(r"C:\Users\dell\Desktop\output_folder\division_asynchrony_phenotype\RNAi_division_asynchrony_Q-value_binary_0.01.txt", index_col= 0,header=0,sep='\t')

division_asynchrony_phenotype_qvalue=pd.read_csv(r"C:\Users\dell\Desktop\output_folder\division_asynchrony_phenotype\\RNAi_division_asynchrony_Q-value.txt", index_col= 0,header=0,sep='\t')

RNAi_division_asynchrony_RMSD = pd.read_csv(r'C:\Users\dell\Desktop\output_folder\division_asynchrony_phenotype\\RNAi_division_asynchrony_RMSD.txt', index_col=0, sep='\t')

CTR_division_timing_vector_mean = pd.read_csv(r'C:\Users\dell\Desktop\output_folder\division_asynchrony_phenotype\CTR_division_timing_vector_mean.txt', index_col=0, sep='\t')

####4.assign the output folder path
Output_Folder = r'C:\Users\dell\Desktop\output_folder\division asynchrony compensation and simulation'  
if not os.path.exists(Output_Folder):
    os.makedirs(Output_Folder)
####################################################################################################################################


replace_1=division_asynchrony_phenotype.replace(-1,1)
replace_1=replace_1.sum(axis=0)
RNAi_celldivision_phenotype_embryo=list(replace_1[replace_1!=0].index)
division_asynchrony_phenotype=division_asynchrony_phenotype[RNAi_celldivision_phenotype_embryo]
selected_embryos=list(RNAi_slope[RNAi_slope['q-value binary']==0].index)
division_asynchrony_phenotype=division_asynchrony_phenotype.loc[selected_embryos].dropna(axis=0,how='all')


total_cell_list=list(division_asynchrony_phenotype.index)
total_cell_list=cell_name_transfer(total_cell_list)

division_asynchrony_phenotype.index=total_cell_list#找到母子细胞对
...
for i in division_asynchrony_phenotype:
    embryo=division_asynchrony_phenotype[i]

    for cell1,cell2 in pair: 
        if embryo.loc[cell1]==1 and  embryo.loc[cell2]==0:
            embryo_name_revise.append(i)
            mother_cell_revise.append(cell1)
            children_cell_revise.append(cell2)
        elif embryo.loc[cell1]==1 and embryo.loc[cell2]==1: 
...
#revise.to_csv(r"C:\Users\dell\Desktop\RNAi_AB_correction_division_embryo_cell_normal_difference.txt", sep="\t")   
#norevise.to_csv(r"C:\Users\dell\Desktop\\RNAi_AB_nocorrection_division_embryo_cell_normal_difference.txt", sep="\t")   

################################################################################################################

cell_list_AB2=['ABa','ABp','EMS','P2']
cell_list_AB4=['ABal','ABar','ABpl','ABpr','MS','E','C']
cell_list_AB8=['ABala','ABalp','ABara','ABarp','ABpla','ABplp','ABpra','ABprp','MSa','MSp','Ca','Cp','P3']
cell_list_AB16=['ABalaa','ABalap','ABalpa','ABalpp','ABaraa','ABarap','ABarpa','ABarpp','ABplaa','ABplap','ABplpa','ABplpp','ABpraa','ABprap','ABprpa','ABprpp','MSaa','MSap','MSpa','MSpp','Ea','Ep','D']
...
def stage_cell_list(children_cell):
    cell=children_cell
    if cell in cell_list_AB2:
        stage_cell_list=cell_list_AB2 
    elif cell in cell_list_AB4:
        stage_cell_list=cell_list_AB4
    elif cell in cell_list_AB8:
        stage_cell_list=cell_list_AB8
    elif cell in cell_list_AB16:
...
order_correction =revise
order_not_correct =norevise
order_info=pd.concat([order_correction,order_not_correct],axis=0)
order_info=pd.merge(order_info,RNAi_slope,how='left',left_index=True,right_index=True)
order_info=order_info[order_info['binary']==0]


embryo_list=list(set(order_info.index))##

abnormal_mother_children_rmsd=pd.DataFrame()
embryo_children_cell=[]
a=0
for i in range(len(order_info.index)):
    a+=1
    print(a)
    temp=order_info.iloc[i]
    embryo=temp.name
    children_cell=temp['children cell']
    embryo_children_cell.append(embryo+'+'+children_cell)
    cell_list_temp=stage_cell_list(children_cell)
    division_vector=Celldivition_RNAi_matrix.loc[cell_list_temp][embryo]

    emb_division=Celldivition_RNAi_matrix.loc[children_cell][embryo]
    emb_cycle=Cellcycle_RNAi_matrix.loc[children_cell][embryo]
    
    CTR_children_cell_cycle=list(Cellcycle_CTR.loc[children_cell])  

    simulate_wt_single_cell_RMSD=[]

    for wt_cycle in CTR_children_cell_cycle:        
        new_division=emb_division-emb_cycle+wt_cycle
        division_vector.loc[children_cell]=new_division
        mean_value=division_vector.max()-division_vector.min()

        value=division_vector.loc[children_cell]
        values=division_vector-value
        cell_vactor_temp=values/mean_value


        cell_vactor_mean=CTR_division_timing_vector_mean[children_cell]
        
        cell_vactor=pd.concat([pd.DataFrame(cell_vactor_temp),pd.DataFrame(cell_vactor_mean)],axis=1)
        cell_vactor=cell_vactor.drop([children_cell]).dropna()

        cell_vactor=cell_vactor.T
        cell_rmsd=scipy.spatial.distance.pdist(cell_vactor)/len(cell_vactor.columns)**0.5
        simulate_wt_single_cell_RMSD.append(cell_rmsd[0])

    abnormal_mother_children_rmsd=pd.concat([abnormal_mother_children_rmsd,pd.DataFrame(simulate_wt_single_cell_RMSD)],axis=1)
abnormal_mother_children_rmsd.index=Cellcycle_CTR.columns
abnormal_mother_children_rmsd.columns=embryo_children_cell
#abnormal_mother_children_rmsd.to_csv(r'C:\Users\dell\Desktop\RNAi_AB_abnormal_mother_children_simulate_rmsd_file1.txt', sep="\t")


#################################随机情况的zscore
embryo_list=list(set(order_info.index))##

abnormal_mother_children_rmsd_simulation=pd.DataFrame()
embryo_children_cell=[]
a=0
for i in range(len(order_info.index)):
    a+=1
    ...
    selected_control=division_asynchrony_phenotype[mother_cell]
    selected_control_embryo=list(selected_control[selected_control==0].index)#选择该母细胞周期不显著变化的胚胎
    random_embryo=random.sample(selected_control_embryo,1)##随机选择其中一个胚胎对后面的过程进行计算，注意随机的胚胎子细胞可能没有RMSD信息需要重新随机
    

    children_cell=temp['children cell']
    embryo_children_cell.append(embryo+'+'+random_embryo[0]+'+'+mother_cell+'+'+children_cell)#原始的胚胎+模拟选择的胚胎+母细胞+子细胞
    cell_list_temp=stage_cell_list(children_cell)
    division_vector=Celldivition_RNAi_matrix.loc[cell_list_temp][random_embryo]#更换成随机胚胎该细胞分裂时间向量

    emb_division=Celldivition_RNAi_matrix.loc[children_cell][random_embryo]#更换成随机胚胎
    emb_cycle=Cellcycle_RNAi_matrix.loc[children_cell][random_embryo]#更换成随机胚胎

    #Cellcycle_CTR_temp=Cellcycle_CTR.drop([embryo],axis=1) #对于野生型模拟去掉所分析的这个胚胎
    CTR_children_cell_cycle=list(Cellcycle_CTR.loc[children_cell])####Cellcycle_CTR_temp and Cellcycle_CTR
    ...
    for wt_cycle in CTR_children_cell_cycle:        
        ...
        mean_value=division_vector.max()-division_vector.min()#用分裂时间最大差值进行标准化
        #mean_value=division_vector.max()
    ...
    abnormal_mother_children_rmsd_simulation=pd.concat([abnormal_mother_children_rmsd_simulation,pd.DataFrame(simulate_wt_single_cell_RMSD)],axis=1)
#abnormal_mother_children_rmsd.index=Cellcycle_CTR.columns
abnormal_mother_children_rmsd_simulation.columns=embryo_children_cell
#abnormal_mother_children_rmsd_simulation.to_csv(r'C:\Users\dell\Desktop\RNAi_AB_abnormal_mother_children_rmsd_control_simulation_file3.txt', sep="\t")
############################

#abnormal_mother_children_rmsd = pd.read_csv(r'C:\Users\dell\Desktop\RNAi_AB_abnormal_mother_children_simulate_rmsd_file1.txt', index_col=0, sep='\t')
#abnormal_mother_children_rmsd_simulation = pd.read_csv(r'C:\Users\dell\Desktop\RNAi_AB_abnormal_mother_children_rmsd_control_simulation_file3.txt', index_col=0, sep='\t')

#XL_RW10434_K10C3.4_20170622_2_s2_emb3+XL_RW10434_UBA-2_20171024_2_s2_emb1+ABprpaap+ABprpaapp
abnormal_mother_children_rmsd_name=[]
rmsd_compare=[]
for i in abnormal_mother_children_rmsd_simulation.columns:
    embryo_name_actural=i.split('+')[0]    
    embryo_name_simulate=i.split('+')[1]
    children_name=i.split('+')[3]
    name_temp=embryo_name_actural+"+"+children_name
    abnormal_mother_children_rmsd_name.append(name_temp)
    #embryo_name_actural='XL_RW10434_K10C3.4_20170622_2_s2_emb3'
    #embryo_name_simulate='XL_RW10434_UBA-2_20171024_2_s2_emb1'
    #children_name='ABprpaapp'
    
    actural_embryo=embryo_name_actural+'+'+children_name
    
    actrual_rmsd=RNAi_division_asynchrony_RMSD[embryo_name_actural].loc[children_name]##原始的RMSD矩阵
    all_simulate_rmsd=abnormal_mother_children_rmsd[actural_embryo] 
    
    difference=all_simulate_rmsd-actrual_rmsd
    compensation_number=difference[difference>0].count()
    all_simulate_rmsd_mean=all_simulate_rmsd.mean()
    all_simulate_rmsd_std = all_simulate_rmsd.std(ddof=1)
    actrual_zscore=(actrual_rmsd-all_simulate_rmsd_mean)/all_simulate_rmsd_std
    
    simulation_rmsd=RNAi_division_asynchrony_RMSD[embryo_name_simulate].loc[children_name]##可更换RMSD矩阵Control_single_cell_RMSD_all or RNAi_single_cell_RMSD_all
    all_simulate_rmsd_control=abnormal_mother_children_rmsd_simulation[i] 
    
    difference_control=all_simulate_rmsd_control-simulation_rmsd
    compensation_number_control=difference_control[difference_control>0].count()
    all_simulate_rmsd_control_mean=all_simulate_rmsd_control.mean()
    all_simulate_rmsd_control_std = all_simulate_rmsd_control.std(ddof=1)
    actrual_zscore_control=(simulation_rmsd-all_simulate_rmsd_control_mean)/all_simulate_rmsd_control_std    
    
    get_values=[compensation_number,compensation_number_control,actrual_zscore,actrual_zscore_control]
    rmsd_compare.append(get_values)

rmsd_compare=pd.DataFrame(rmsd_compare)
rmsd_compare.columns=['compensation_number','compensation_number_control','actrual_zscore','actrual_zscore_control']
rmsd_compare.index=abnormal_mother_children_rmsd_name

emb=[]
cell=[]
for i in rmsd_compare.index:
    emb_t=i.split('+')[0]
    cell_t=i.split('+')[1]
    emb.append(emb_t)
    cell.append(cell_t)
    
rmsd_compare.index=emb
rmsd_compare['children cell']=cell
#rmsd_compare.to_csv(r'C:\Users\dell\Desktop\rmsd_compare_zscore_simulation_control_file4.txt', sep="\t")

############################################################################################################


#division_compensation=pd.read_csv(r"C:\Users\dell\Desktop\rmsd_compare_zscore_simulation_control_file4.txt", index_col= 0,header=0,sep='\t')
division_compensation=rmsd_compare
children_cell=list(division_compensation['children cell'])
mother_cells=[]
for i in cell_name_transfer(children_cell):
    mother_cell=i[0:-1]
    mother_cells.append(mother_cell)
mother_cells=cell_name_transfer(mother_cells)
division_compensation['mother cell']=mother_cells
#division_compensation.to_csv(r'C:\Users\dell\Desktop\division_timing_compensation_AB.txt', sep="\t")
#######
...
a=0
for i in range(len(division_compensation.index)):
    a+=1
    print(a)
    #i=2233       #XL_RW10434_JMJC-1_20170612_3_s1_emb1胚胎D cell的位置
    temp=division_compensation.iloc[i]
    embryo=temp.name
    children_cell=temp['children cell']
    mother_cell=temp['mother cell']
    ...
    division_asynchrony_temp=division_asynchrony_phenotype.loc[detected_cell_list][embryo]
    division_asynchrony_temp_mother=division_asynchrony_phenotype.loc[detected_cell_list_mother][embryo]

    reference_cell_number=division_asynchrony_temp.count()
    reference_cell_number_mother=division_asynchrony_temp_mother.count()
    
    abnormal_cell_number=division_asynchrony_temp[division_asynchrony_temp==1].count()
    abnormal_cell_number_mother=division_asynchrony_temp_mother[division_asynchrony_temp_mother==1].count()
...
#division_compensation['reference_cell_number_children'] =   reference_cell_number_all
#division_compensation['abnormal_cell_number_children']  =  abnormal_cell_number_all
division_compensation['abnormal_rate_children'] = abnormal_rate_all
#division_compensation['reference_cell_number_mother'] =   reference_cell_number_all_mother
#division_compensation['abnormal_cell_number_mother']  =  abnormal_cell_number_all_mother
#division_compensation['abnormal_rate_mother'] = abnormal_rate_all_mother

#division_compensation.to_csv(r'C:\Users\dell\Desktop\division_timing_compensation_screen_AB_file5.txt', sep="\t")


division_asynchrony_compensation_withqvalue_merge=division_compensation
cut_off=scipy.stats.norm.ppf(0.95)
children_qvalue_all=[]
mother_qvalue_all=[]
compensation_actrual=[]
compensation_control=[]
a=0
for i in range(len(division_asynchrony_compensation_withqvalue_merge.index)):
    a+=1
    print(a)
    #i=2233       #XL_RW10434_JMJC-1_20170612_3_s1_emb1胚胎D cell的位置
    temp=division_asynchrony_compensation_withqvalue_merge.iloc[i]
    embryo=temp.name
    children_cell=temp['children cell']
    mother_cell=temp['mother cell']

    children_qvalue=division_asynchrony_phenotype_qvalue.loc[children_cell][embryo]
    mother_qvalue=division_asynchrony_phenotype_qvalue.loc[mother_cell][embryo]
    
    children_qvalue_all.append(children_qvalue)
    mother_qvalue_all.append(mother_qvalue)
    compensation_actrual_temp=temp['actrual_zscore']
    if compensation_actrual_temp < -cut_off:
        compensation_actrual.append(1)
    else:
        compensation_actrual.append(0)
        
    compensation_control_temp=temp['actrual_zscore_control']
    if compensation_control_temp < -cut_off:
        compensation_control.append(1)
    else:
        compensation_control.append(0)    
division_asynchrony_compensation_withqvalue_merge['mother_qvalue'] =   mother_qvalue_all
division_asynchrony_compensation_withqvalue_merge['children_qvalue']  =  children_qvalue_all

division_asynchrony_compensation_withqvalue_merge['compensation_actrual'] =   compensation_actrual
division_asynchrony_compensation_withqvalue_merge['compensation_control']  =  compensation_control

division_asynchrony_compensation_withqvalue_merge

division_asynchrony_compensation_withqvalue_merge.to_csv(os.path.join(Output_Folder,'division_asynchrony_compensation_analysis.txt'), sep="\t")
