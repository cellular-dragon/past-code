# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 14:04:53 2022
...
location_cluster=pd.read_csv(r"C:\Users\dell\Desktop\substitude_data\cell pair abnormal rate\location_cluster.txt", index_col= 0,header=0,sep='\t')
location_cluster_list=list(set(location_cluster['Localization-cluster']))
location_cluster_list.sort()

location_fate_combine_cluster=pd.read_csv(r"C:\Users\dell\Desktop\substitude_data\cell pair abnormal rate\tissue_cell_contain_cell.txt", index_col= 0,header=0,sep='\t')
cluster_list=list(set(location_fate_combine_cluster.index))
cluster_list.sort()


def find_cluster(cell):
    for i in cluster_list:
        cluster_cell_name=list(location_fate_combine_cluster.loc[i]['cell'])
        if cell in cluster_cell_name:            
            return cluster_cell_name,i

def find_location_cluster(cell):
    for i in location_cluster_list:
        cluster_cell_name=list(location_cluster[location_cluster['Localization-cluster']==i].index)
        if cell in cluster_cell_name:            
            return cluster_cell_name,i

def find_mother(cell):
    cell=cell_name_transfer(cell) 
    mother_cell=cell[:-1]
    mother_cell=cell_name_transfer(mother_cell) 
    return mother_cell

#################################################################################################################
#input cell pair distance phenotype folder from “16. cell pair distance phenotype”
cell_pair_distance_phenotype_path = os.path.abspath(r'C:\Users\dell\Desktop\output_folder\cell_pair_distance_phenotype')

##input cell pair distance phenotype folder from “9. relative cell position phenotype analysis”
RNAi_cellposition_qvalue_binary=pd.read_csv(r"C:\Users\dell\Desktop\output_folder\RNAi_relative_cell_position_Q-value_binary_0.01.txt", index_col= 0,header=0,sep='\t')

#####output path and file name
Output_Folder = r'C:\Users\dell\Desktop\output_folder'   # assign the output folder path
output_file_name='cell module cell pair distance rate' #input value file name
#################################################################################################################
os.chdir(cell_pair_distance_phenotype_path) 
Matrix_list=os.listdir(cell_pair_distance_phenotype_path)

gene_cluster_cell_compare=pd.DataFrame()
gene_name=[]
a=0
for i in Matrix_list:
    #i='XL_RW10434_DRH-3_20190311_3_s2_emb1_AB256_comparison_Matrix.txt'
    a+=1
    print(a)
    
    name=i.split('_')[4]#
    
    embryo_name=i.split('_AB256_')[0]#new
    
    
    compare_matrix= pd.read_csv(i, index_col= 0,header=0,sep='\t')   
    compare_matrix=abs(compare_matrix)

    compare_matrix=compare_matrix.dropna(axis=0,how='all')
    compare_matrix=compare_matrix.dropna(axis=1,how='all')
    
    exist_cell_name=list(compare_matrix.index)

    cell_number=len(exist_cell_name)
    
    if cell_number/361 > 0.9:
        gene_name.append(name)
        
        phenotype_temp=RNAi_cellposition_qvalue_binary.loc[exist_cell_name][embryo_name]
        
        cell_with_phenotype=list(phenotype_temp[phenotype_temp==1].index)
        
        
        if len(cell_with_phenotype)>0:
            
            analysis_cell_with_phenotype=[]
            for cell in cell_with_phenotype:
                mother_cell=find_mother(cell)
                mother_cell_phenotype=RNAi_cellposition_qvalue_binary.loc[mother_cell][embryo_name]
                if mother_cell_phenotype==0:
                    analysis_cell_with_phenotype.append(cell)
                    
            if len(analysis_cell_with_phenotype)>0:
                abnormal_cell_rate=len(analysis_cell_with_phenotype)/cell_number
                cluster_compare=pd.DataFrame()
                for cell in analysis_cell_with_phenotype:
                    #cell='ABplappaap'
                    fate_cluster_cell_info=find_cluster(cell)
                    location_cluster_cell_info=find_location_cluster(cell)
                    
                    if fate_cluster_cell_info is not None:
                        abnormal_cell=cell
                        fate_cluster_cell_name=fate_cluster_cell_info[0]
                        fat_cluster_name=fate_cluster_cell_info[1]
                        
                        location_cluster_cell_name=location_cluster_cell_info[0]
                        location_cluster_name=location_cluster_cell_info[1]
                    
                        if set(fate_cluster_cell_name)<=set(compare_matrix.index):
                    
                            fate_cluster_matrix=compare_matrix.loc[fate_cluster_cell_name].dropna(axis=0,how='all')
                            fate_cluster_matrix=fate_cluster_matrix[fate_cluster_matrix.index]            
                            detected_cell_number=len(fate_cluster_matrix)            
                            abnormal_pair_1=fate_cluster_matrix.loc[cell].sum()
                            cell_pair_1=detected_cell_number-1
                            abnormal_distance_fate_cluster=abnormal_pair_1/cell_pair_1      
                                                                              
                            other_location_cluster_cells=list(set(location_cluster_cell_name).difference(set(fate_cluster_cell_name)))  #in location cluster not in fate cluster      
                            other_location_cluster_cells.append(cell) 
                            location_cluster_matrix_other=compare_matrix.loc[other_location_cluster_cells].dropna(axis=0,how='all')
                            location_cluster_matrix_other=location_cluster_matrix_other[location_cluster_matrix_other.index]                           
                            detected_cell_number_2=len(location_cluster_matrix_other)            
                            abnormal_pair_2=location_cluster_matrix_other.loc[cell].sum()
                            cell_pair_2=detected_cell_number_2-1
                            if cell_pair_2==0:
                                abnormal_distance_other_location_cluster=np.nan
                                
                            else:
                                abnormal_distance_other_location_cluster=abnormal_pair_2/cell_pair_2                         
                            
                            fate_location_cluster_cell_name=list(set(fate_cluster_cell_name).union(set(location_cluster_cell_name)))
                            other_cells=list(set(exist_cell_name).difference(set(fate_location_cluster_cell_name)))  #not in location cluster and fate cluster

                            other_cells.append(cell)            
                            cluster_matrix_other=compare_matrix.loc[other_cells].dropna(axis=0,how='all')
                            cluster_matrix_other=cluster_matrix_other[cluster_matrix_other.index]            
                            detected_cell_number_3=len(cluster_matrix_other)            
                            abnormal_pair_3=cluster_matrix_other.loc[cell].sum()
                            cell_pair_3=detected_cell_number_3-1
                            abnormal_distance_other=abnormal_pair_3/cell_pair_3                             

                            random_abnormal_percent=[]                     #random 100 times  
                            remove_cell=list(set(other_cells).difference(set([cell])))              
                            for i in range(100):
                                random_cluster_cell_name=random.sample(remove_cell, detected_cell_number-1)
                                random_cluster_cell_name.append(cell)
                                random_cluster_matrix=compare_matrix.loc[random_cluster_cell_name][random_cluster_cell_name]                           
                                random_abnormal_percent_temp=random_cluster_matrix.loc[cell].sum()/(detected_cell_number-1)            
                                random_abnormal_percent.append(random_abnormal_percent_temp)
                            random_abnormal_mean=np.mean(random_abnormal_percent)
                            random_abnormal_std=np.std(random_abnormal_percent)          
                        
                            cluster_compare_temp=pd.DataFrame([fat_cluster_name,abnormal_pair_1,cell_pair_1,abnormal_distance_fate_cluster,
                                                               location_cluster_name,abnormal_pair_2,cell_pair_2, abnormal_distance_other_location_cluster,
                                                               abnormal_pair_3,cell_pair_3,abnormal_distance_other,
                                                               random_abnormal_mean,random_abnormal_std,
                                                               embryo_name,abnormal_cell,abnormal_cell_rate])
                            cluster_compare_temp=cluster_compare_temp.T
                            cluster_compare_temp.columns=['fate_cluster_name','fate_abnormal_pair_cluster','fate_detected_pair_cluster','fate_cluster_abnormal',
                                                          'location_cluster_name','other_location_abnormal_pair_cluster','other_location_detected_pair_cluster','other_location_cluster_abnormal',
                                                          'abnormal_pair_other','detected_pair_other','other_abnormal',
                                                          'random_abnormal_mean','random_abnormal_std',
                                                          'embryo name','abnormal cell','abnormal_cell_rate']  
                            
                            cluster_compare_temp=cluster_compare_temp.set_index(['embryo name'])
                            cluster_compare=pd.concat([cluster_compare,cluster_compare_temp],axis=0)
                gene_cluster_cell_compare=pd.concat([gene_cluster_cell_compare,cluster_compare],axis=0)
#gene_cluster_cell_compare.to_csv(r"C:\Users\dell\Desktop\fate_clone_cluster_compares.txt", sep="\t")

 
##############

data_reshape=gene_cluster_cell_compare
clusters=list(set(data_reshape['clone']))
clusters.sort()

cluster_compares=[]
for i in clusters:
    temp_value=data_reshape[data_reshape['clone']==i]
    actrual=temp_value[temp_value['type']=='fate clone cells']['value']
    other_location=temp_value[temp_value['type']=='other location cluster cells']['value']
    other=temp_value[temp_value['type']=='other cells']['value']
    
    df1=len(actrual)-1
    df2=len(other_location)-1
    df3=len(other)-1
    ci_1 = stats.t.interval(0.95, df1, loc=0, scale=stats.sem(actrual))[1]
    ci_2 = stats.t.interval(0.95, df2, loc=0, scale=stats.sem(other_location))[1]
    ci_3 = stats.t.interval(0.95, df2, loc=0, scale=stats.sem(other))[1]
    
    actrual_mean=actrual.mean()
    other_location_mean=other_location.mean()
    other_mean=other.mean()
    
    cluster_compare=[actrual_mean,other_location_mean,other_mean,ci_1,ci_2,ci_3]
    cluster_compares.append(cluster_compare)
cluster_compares=pd.DataFrame(cluster_compares)    
cluster_compares.index=clusters
cluster_compares.columns=['actrual_mean','other_location_mean','other_mean','actrual_95CI','other_location_95CI','other_95CI']

cluster_compares.to_csv(os.path.join(Output_Folder,output_file_name+".txt"), sep="\t")  